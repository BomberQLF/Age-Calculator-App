document.addEventListener("DOMContentLoaded", () => {
   ...
   // Function to animate the age values with GSAP librairy
   function animateValue(element, endValue) {
     const startValue = parseInt(element.textContent, 10);
     gsap.fromTo(element, { innerHTML: startValue }, {
       innerHTML: endValue,
       duration: 2,
       ease: "power2.out",
       modifiers: {
         innerHTML: (innerHTML) => {
           return Math.floor(innerHTML);
         }
       }
     });
   }
   ...
   form.addEventListener('submit', (e) => {
     ...
     if (!isNaN(year) && !isNaN(month) && !isNaN(day)) {
       ...
       // Animate the age values using GSAP
       animateValue(spanYears, ageYears);
       animateValue(spanMonths, ageMonths);
       animateValue(spanDays, ageDays);
